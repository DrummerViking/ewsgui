...
$parentModule = 'O365HybridLab'
if (-not $ModuleName) { $ModuleName = 'O365HybridLab.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\O365HybridLab'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\O365HybridLab.psd1"
$prereqHash = @{
