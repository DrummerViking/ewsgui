...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'EWSGui.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'EWSGui.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'EWSGui.Client.ProtectedToken'
